...
def expenditures(y,p,alpha,beta,phi,NegativeDemands=True,tol=1e-3):
...
def budgetshares(y,p,alpha,beta,phi,NegativeDemands=True,tol=1e-3):
    
    n,alpha,beta,phi = check_args(p,alpha,beta,phi)
    
    x=expenditures(y,p,alpha,beta,phi,NegativeDemands=NegativeDemands,tol=tol)
