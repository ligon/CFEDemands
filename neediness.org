  ...
  def estimate_bdce_with_missing_values(y,z,market=None,prices=None,return_v=False,return_se=False, time_index=1):
      ...
      for i in range(n):
          ...
          if not market is None:
              ...
              foo=pd.Series(["-".join(map(str,tuple(x))) for x in pd.concat([use_indices(Wz,[t])[t],market],axis=1,join='inner').as_matrix().tolist()],index=Wz.index,name=t+'-market')
      ...
      if not market is None: #~ ELLIOTT (2016-11-07): Split stringified time-market index into multi-index.
          ...
          b.index.name=t
          b = b.reset_index()
          b['market'] = b[t].apply(lambda x: x.split("-")[time_index-1])
          try: b['market'] =b['market'].apply(int) 
