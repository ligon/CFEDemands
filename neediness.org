...
#+begin_src python :noweb no-export :results output :tangle test_estimate_with_missing.py :exports none
  ...
  logL=logL.unstack()
  logL0=np.log(truth['lambdas'][0]).unstack()
  foo=pd.Panel({"L0":logL0,"Lhat":logL})
  print "Correlation of log lambda with estimate (after normalization): (%f,%f)" % tuple([foo[:,:,t].corr().iloc[0,1] for t in logL.columns])
