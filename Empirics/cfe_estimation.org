...
* Distance between two estimates of \beta
  ...
  elasticity, as it relates the price and Frisch elasticities).  Thus, if we have /two/ different
  ...
  \end{equation}
  We adopt a sort of $L^2$ norm for measuring the distance between these two random vectors,
  defining 
  \label{eq:norm}
     {} ||\vec{x}||_W = \vec{x}^\T \vec{W}\vec{x},
  \end{equation}
  where $W$ is some postive definite matrix.  An /optimal/ choice of
  $\vec{W}$, in  a GMM sense, is to use
  $\vec{W}=\mbox{Cov}(\vec{x})^{-1}$.  Absent prior knowledge
  regarding this  covariance matrix, in the present case we observe
  that \(\mbox{Cov}(\psi\beta^1 - \beta^2) = \psi^2V^1 + V^2\), where
  $V^1$ and $V^2$ are the covariance matrices corresponding to
  $\beta^1$ and $\beta^2$.  Supposing that estimates of these two covariance
  matrices can be obtained at the same time $\beta^1$ and $\beta^2$
  are estimated, we choose $\psi$ to minimize 
  \begin{equation}
  \label{eq:min_chi2}
     H(\beta^1,\beta^2,V^1,V^2) = \min_\psi \left(\psi\beta^1 - \beta^2\right)^\T\left[\psi^2 V^1 + V^2\right]\left(\psi\beta^1 - \beta^2\right).
  \end{equation}
  If the random variables $\beta^1$ and $beta^2$ are normally
  distributed, then the estimates $V^1$ and $V^2$ will have a Wishart
  distribution, and the statistic $H$ will be distributed as
  Hotelling's $T^2$ with a single degree of freedom.
...
baz=broadcast_binary_op(foo, lambda x,y: x+y, bar,how='left')
