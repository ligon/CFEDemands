...
#+BEGIN_SRC python :noweb no-export :results output :tangle ../cfe/estimation.py
    ...
    def estimate_reduced_form(y,z,return_v=False,return_se=False,VERBOSE=False):
      ...
      for i,Item in enumerate(y.columns):
          ...
          a[Item] = y[Item].mean() - d[Item].dot(z.mean(axis=0)) - np.array(b[Item]).dot(DateLocD.mean().values)
...
#+BEGIN_SRC python :var T=2 N=3000 n=6 :tangle ../cfe/test/estimate_reduced_form.py
<<lambdas_dgp>>
<<characteristics_dgp>>
<<prices_dgp>>
<<expenditures_dgp>>
...
J=g.T*A*g
assert np.all(np.abs(d-1)<0.1), "Coefficients on characteristics should all be equal to 1"
...
#+begin_src python :noweb no-export :results output :tangle ../cfe/estimation.py
  ...
  def bootstrap_elasticity_stderrs(e,tol=1e-2,minits=30,return_samples=False,VERBOSE=False,outfn=None,TRIM=True):
