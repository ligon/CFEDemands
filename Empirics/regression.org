...
#+begin_src python
    ...
    MdMp = lambda self,x: Md_generator(x,
                                  self.d,
                                  method=self.attrs['method'],Mp=True)

    Md = lambda self,x: Md_generator(x,
                                self.d,
                                method=self.attrs['method'],Mp=False)
...
#+begin_src python
    def validate(self,rectify=False,GramSchmidt=False,verbose=False):
        ...
        if rectify:
            ...
            self.pi_se = self.pi_se*(B['pi']@self.y.groupby('j').count()/self.y.shape[0])
            try:
                self.gamma = (self.gamma.stack()*B['gamma_d']).unstack('k')
                if self.gamma_se is not None:
                    self.gamma_se = (self.gamma_se.stack()*B['gamma_d']).unstack('k')
            except AttributeError:
                self.gamma = self.gamma*B['gamma_d']
                if self.gamma_se is not None:
                    self.gamma_se = self.gamma_se*B['gamma_d']
        return B
