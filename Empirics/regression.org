...
def estimate_beta(MdMpy,
                  ...
                  return_se=False,
                  return_var=False,bootstrap_tol=None,Mdp=None,verbose=False):
    ...
    if (return_se or return_var) and bootstrap_tol:
...
def _estimation(y,d,K=None,beta=None,bootstrap_tol=None,return_se=False,rectify=False,verbose=False):
...
#+begin_src python
    ...
    def get_gamma_d(self,verbose=None):
        ...
        b,Mpw,seb,mults,mults_se,e1 = estimate_beta_and_Mpw(self.y,MdMp,
                                      return_se=self.attrs['return_se'],
                                      bootstrap_tol=self.attrs['bootstrap_tol'],
                                      verbose=verbose)

        if self.b is None: self.b = b.squeeze()
        ...
        gamma,gamma_d,e2 = estimate_gamma(Mp(self.y),self.b,self.Mpw,self.d,
    ...
    def get_w(self,verbose=None):
        ...
        y0 = (Mpi(self.y) - Mp(gamma_d)).dropna()
    ...
    def get_pi(self,verbose=None):
        ...
        self.pi = estimate_pi(self.y,b,w,Ar,gamma_d,verbose=verbose)

        return self.pi


    def predicted_log_expenditures(self,fill_missing=True,verbose=None):
        """
