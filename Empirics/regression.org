...
#+begin_src python :tangle no
    ...
    MdY = (pd.DataFrame(Y) - Yg.transform(np.mean)).squeeze()
    ...
    if shuffles:
        ...
        for s in range(shuffles):
            ...
            Valt.append((pd.DataFrame(Y) - Yg_alt.transform(np.mean)).squeeze().var())
...
Construct =Md= operator.  This can involve strictly categorical variables, in which case we can construct a non-parametric estimator of $\E(X|d=\tau)$ simply by taking means for every value of $\tau$ observed in the sample.    When the method is linear, we instead
#+begin_src python
def Md_generator(X,d,method='categorical',expected=False):
    ...
    if method=='categorical':
        ...
        gamma_d = Xg.transform(np.mean).squeeze()
        ...
        d = pd.DataFrame(d)
        if np.all(d.std()>0): d['Constant'] = 1
        ...
        if 'j' in X.index.names:
            ...
            if expected: # Really return gamma_d,gamma
                ...
                Gamma = {}
                for j,y in Xj:
                    EdX[j],Gamma[j] = Md_generator(y.droplevel('j'),d,method='linear',expected=True)
                ...
                MdX = {}
                for j,y in Xj:
                    MdX[j] = Md_generator(y.droplevel('j'),d,method='linear',expected=False)
            ...
            gamma_d.name = 'gamma_d'
            if expected: return gamma_d,pd.Series(gamma.squeeze(),index=x.columns)
        ...
        if len(cats):
            Z = Z - Z.groupby(cats).transform(np.mean)
        else:
            Z = Z - Z.mean()
...
#+end_src
** \(\mathcal{M}_{(d,p)}\) operator

#+begin_src python :tangle no
def Mdp_generator(X,tau,method='categorical'):
    """
    Md operator, for either categorical or linear expectations.
    ...
    if method=='categorical': # assuming conditioning is on groups tau
        ...
        if 'j' in X.index.names:
            ...
            if len(group):
                X = X - X.groupby(group).transform(np.mean)
            else:
                X = X - X.mean()
    ...
    if return_se and bootstrap_tol:
        if bootstrap_tol is None:
            ...
            while its < 30 or np.linalg.norm(seb-last) > bootstrap_tol:
                ...
                while not okay:
                    try:
                        B[its] = estimate_beta(MpMdY.groupby(['t','m']).sample(frac=1,replace=True))[0]
                        okay = True
...
#+begin_src python
    ...
    def get_beta(self,verbose=None,return_se=None,bootstrap_tol=1e-2,heteroskedastic=False):
        ...
        b, seb, V = estimate_beta(MpMdy,verbose=verbose,return_se=return_se,bootstrap_tol=bootstrap_tol,heteroskedastic=heteroskedastic)
        b = b.squeeze()
        if seb is not None:
...
def main():
    ...
    r = rgsn.Regression(y=y,d=d,return_se=False,K=None,bootstrap_tol=0.005,verbose=False)
