...
#+name: code:rectify_w
#+begin_src python
def rectify_w(y,gamma_d,beta,w,Ar,max_iter=10,tol=1e-4,
              warn_band=(0.5,2.0),verbose=False):
    ...
    if not (warn_band[0] <= Delta <= warn_band[1]):
        warnings.warn(
...
#+begin_src python
    ...
    def rectify(self,inplace=False,max_iter=10,tol=1e-4,warn_band=(0.5,2.0),verbose=False):
        """PROTOTYPE: pin the global scale of w (see rectify_w).

        Iterates a global slope delta on beta*w (recomputing A(r) and pi each
        pass) and rescales w_tilde = Delta*w.  Not on the main path; with
        inplace=True it overwrites self.w/self.Ar/self.pi (and scales w-related
        standard errors by Delta) and clears cached predictions.

        Returns the diagnostic (Delta, delta_history).
        """
        ...
        w2,Ar2,pi2,Delta,history,e3 = rectify_w(self.y,gamma_d,beta,w,Ar,
                                                ...
                                                warn_band=warn_band,verbose=verbose)
        if inplace:
            self.w = w2
            self.Ar = Ar2
            self.pi = pi2
