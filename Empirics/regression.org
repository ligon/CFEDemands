...
def estimate_beta_and_Mpw(y,Mdp,return_se=False,bootstrap_tol=None,verbose=False):
    ...
    mults = pd.Series(coeffs[len(cols):],index=pd.MultiIndex.from_tuples([tm[1:] for tm in TM]),name='mult')
    ...
    if return_se: # See Greene-Seaks (1991)
        with warnings.catch_warnings():
            ...
            se_mult = pd.Series(se_mult,index=pd.MultiIndex.from_tuples([tm[1:] for tm in TM]),name='se_mult')
    else:
        se_mult = None
        e1 = None
