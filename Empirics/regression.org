...
#+name: code:rectify_w
#+begin_src python
def rectify_w(y,gamma_d,beta,w,Ar,max_iter=20,tol=1e-4,
    ...
    missingness); max_iter defaults to 20.  Sign flip (delta<=0) raises;
    non-convergence warns.  warn_band, if given, warns when the cumulative
    Delta leaves the band -- but the economic+statistical gate lives in
    Regression.rectify (which has the delta s.e.).
    ...
    for it in range(max_iter):
        ...
        lhs = L - offset(Ar)
        # FWL: partial the (t,m) level (pi) out of both sides, read delta off beta*w
        lhs_d = lhs - lhs.groupby(['t','m']).transform('mean')
        bw_d  = bw  - bw.groupby(['t','m']).transform('mean')
        delta = (lhs_d*bw_d).sum()/(bw_d**2).sum()
        ...
        # Recompute A(r): (t,m,j) mean of (y - gamma_d - beta*w), centered over j.
        bw = outer(w,beta).stack().reindex(L.index)
        Ar = (L - bw).groupby(['t','m','j']).mean()
        Ar = Ar - Ar.groupby(['t','m']).transform('mean')
