...
class Result(xr.Dataset):
    ...
    def iterated_estimation(self,VERBOSE=False,tol=1e-3):
        ...
        if is_none(self.loglambdas):
            ...
            d['logL'] = 0
            while np.abs(d_last - d).max().max() > tol:
                
                if VERBOSE: 
                    print("Norm: %f" % np.linalg.norm(d_last-d))
                d_last = d
        ...
        a,ce,d,sed,sea,V = estimation.estimate_reduced_form(y,rhs,return_se=True,return_v=True,VERBOSE=False)
    ...
    def anova(self):
        ...
        y = self.y drop_useless_expenditures(as_df=True) # A dataframe
