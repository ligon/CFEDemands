...
class Result(xr.Dataset):
    ...
    def iterated_estimation(self,VERBOSE=False,tol=1e-3):
        ...
        if is_none(self.loglambdas):
            ...
            its = 0
            while (np.abs(d_last - d).max().max() > tol) or (its < 2):
                
                if VERBOSE: 
                    print("Iteration %d, Norm: %f" % (its,np.linalg.norm(d_last-d)))
                d_last = d
                bphi,logL = estimation.get_loglambdas(ce,TEST=False,min_obs=min_obs)
                #logL = logL + dlogL
                rhs = to_dataframe(self.z,'k')
                rhs['logL'] = logL.astype(float)

                a,ce,d = estimation.estimate_reduced_form(y,rhs,return_se=False,return_v=False,VERBOSE=False)
                if VERBOSE: 
                    print(d)
                #ce.dropna(how='all',inplace=True)
                ce = y  - DateLocD@a.T - z@d.drop('logL',axis=1).T
                its += 1

        a,e,d,sed,sea,V = estimation.estimate_reduced_form(y,rhs,return_se=True,return_v=True,VERBOSE=False)
...
#+begin_src python :results output :var N=500 T=3 M=1 n=12 k=3 :tangle ../cfe/test/test_iterated_estimation.py
import cfe
import numpy as np
#import warnings
