...
class Result(xr.Dataset):
    ...
    def get_stderrs(self,as_df=True,return_v=False):
        if is_none(self.se_beta):
            ...
            se,V = estimation.bootstrap_elasticity_stderrs(ce,return_v=return_v,tol=tol,VERBOSE=VB)
            self['se_beta'] = from_dataframe(se)
            self['beta_covariance'] = xr.DataArray(V.values,dims=['i','ip'],coords={'i':self.coords['i'],'ip':self.coords['i']})
...
*** Test demands
#+name: test_demands
#+begin_src python :results output :tangle ../cfe/test/test_demands.py
import cfe
import numpy as np
