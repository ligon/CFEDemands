...
from cfe.df_utils import to_dataframe, is_none, from_dataframe, ols, df_norm, use_indices
...
class Result(xr.Dataset):
    ...
    __slots__ = ['y','z','delta','beta','alpha','loglambdas','a','yhat','ce',
                 ...
                 'useless_expenditures','stderr_tol','indices','iterate','verbose','all_tm']
    ...
    def iterated_estimation(self,VERBOSE=False,tol=1e-3):
        """Estimate (delta,beta,loglambda).

        Sets beta, loglambdas, and cehat.
        """
        def my_ols(y,DateLocD,z,logL=None,return_se=False,return_v=False,VERBOSE=False):
            ...
            for i,Item in enumerate(y.columns):
                ...
                if logL is None:
                    ...
                    lhs,rhs=drop_missing([y.iloc[:,[i]],pd.concat([z,DateLocD,logL],axis=1)])
            ...
            a[Item] = myb.loc[useDateLocs].squeeze()  # Terms involving prices
        ...
        if is_none(self.loglambdas):
            ...
            logL = 0
            coeffs_last = 0
            d = to_dataframe(self.delta,'k')
            d['logL'] = 0

            a = to_dataframe(self.a,'i').T

            coeffs = pd.concat([a,d],axis=1)

            its = 0
            while (np.abs(coeffs_last - coeffs).max().max() > tol) or (its < 3):
                
                if VERBOSE: 
                    print("Iteration %d, Norm: %f" % (its,np.linalg.norm(coeffs_last-coeffs)))
                coeffs_last = coeffs
                ...
                #a,ce,d = estimation.estimate_reduced_form(y,rhs,return_se=False,return_v=False,VERBOSE=False)
                coeffs, e = my_ols(y,DateLocD,z,logL,return_se=False,return_v=False,VERBOSE=VERBOSE)

                coeffs = coeffs.drop('logL',axis=1)
                
                if VERBOSE: 
                    print(coeffs)
                #ce.dropna(how='all',inplace=True)
                ce = e + logL@coeffs['logL'].T  

                its += 1

        coeffs,e,sed,sea,V = my_ols(y,DateLocD,z,logL,return_se=True,return_v=True,VERBOSE=False)

        a = coeffs.loc[DateLocD.columns]
        d = coeffs.loc[z.columns]
        bphi = coeffs['logL']

        seb = sed['logL']
        sed = sed.drop('logL',axis=0)
        ...
        self['loglambdas'] = logL.astype(float).to_xarray()
...
x,truth = cfe.dgp.expenditures(N,T,M,n,k,beta)

r = cfe.Result(y=np.log(x),z=truth.characteristics)

r.iterated_estimation(VERBOSE=True)
