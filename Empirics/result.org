...
class Result(xr.Dataset):
    ...
    def iterated_estimation(self,VERBOSE=False,tol=1e-3):
        ...
        """
        def my_ols(y,z,loglambda=None,return_se=False,return_v=False,VERBOSE=False):
            ...
            for i,Item in enumerate(y.columns):
                ...
                b[Item] = myb.squeeze()
                ce[Item] = mye # Drop constraint that sums time-effects to zero
        ...
        if is_none(self.loglambdas):
            ...
            self.drop_useless_expenditures() # as_df=True) # Returns a dataframe
            y = self.y.dropna('i',how='all')

            ce = y - self.z@self.delta - self.a.mean('i')
            ...
            dm = sorted(set(zip(dm['t'],dm['m'])))

            its = 0
            while (its<3) or (np.abs(coeffs_last - coeffs).max().max() > tol):
                ...
                coeffs, e = my_ols(y,z,loglambda,return_se=False,return_v=False,VERBOSE=VERBOSE)
                ...
                coeffs = coeffs.drop('loglambda',axis=1)
                
                if VERBOSE: 
                    print(coeffs)
                #ce.dropna(how='all',inplace=True)
                ce = y - z@delta.T - a.groupby(['t','m']).mean()

                its += 1

        coeffs,e,se,V = my_ols(y,z,loglambda,return_se=True,return_v=True,VERBOSE=False)
        ...
        self['alpha'] = self.a.sel(t=s.firstround,drop=True)
        self['se_alpha'] = self.se_a.sel(t=s.firstround,drop=True)
...
r = cfe.Result(y=np.log(x),z=truth.characteristics,iterate=True)
